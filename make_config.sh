...
cp -f /root/openvpn_server/vars_example/vars_30/vars /root/CA/EasyRSA-v3.0.6
cd /root/openvpn_server/EasyRSA-v3.0.6/
printf "\n" | ./easyrsa gen-req $cert_name nopass
cp /root/openvpn_server/EasyRSA-v3.0.6/pki/private/$cert_name_key /root/client-configs/keys/
cp /root/openvpn_server/EasyRSA-v3.0.6/pki/reqs/$cert_name_req /tmpCA/
cd /root/CA/EasyRSA-v3.0.6/
printf "\n" | ./easyrsa import-req /tmpCA/$cert_name_req $cert_name
printf "yes\n" | ./easyrsa sign-req client $cert_name
cp /root/CA/EasyRSA-v3.0.6/pki/issued/$cert_name_crt /tmp
cp /tmp/$cert_name_crt /root/client-configs/keys/
cp /root/openvpn_server/EasyRSA-v3.0.6/ta.key /root/client-configs/keys/
